...
class BoobleSpider(BaseEventSpider):
    """
    Spider to scrape events from booble.it.
    NOTE: Selectors are placeholders and need to be defined based on website structure.
    """
    name = "booble"
    allowed_domains = ["booble.it"]
    start_urls = [
        "https://www.booble.it/it/eventi-e-spettacoli-38/",
        "https://www.booble.it/it/musica-cinema-e-teatro-48/"
    ]

    # --- Selectors based on ilvescovado structure ---
    ...
    _items_scraped_count = 0 # Counter for scraped items per start URL
    ...
    def parse(self, response, **kwargs):
        ...
        # Reset counter if it's a new start_url being processed (simple check)
        # A more robust way might involve passing meta info if needed
